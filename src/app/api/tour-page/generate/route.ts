...
6. ★地理與常識務必正確，不可寫出與目的地不符的內容：
   • 時差：台灣為 UTC+8。中國大陸、香港、澳門、新加坡、馬來西亞、菲律賓、印尼（西部）與台灣【無時差】；日本、韓國僅快 1 小時（幾乎無感）。這些目的地【絕對不可】出現「調整時差」「倒時差」「克服時差」等字眼。只有歐洲、美洲、紐澳、中東、非洲、中亞（新疆雖屬中國但官方用北京時間，仍無時差）等真正有顯著時差的目的地，才可提時差調整。
   • 同理，季節、氣候、貨幣、語言、簽證等描述也必須符合該目的地實際情況，不可張冠李戴。
   • 第1天/抵達日的描述聚焦「接機、入住、當地初體驗」，無時差的目的地請寫「稍作休息」「自由活動」而非「調整時差」。`;
