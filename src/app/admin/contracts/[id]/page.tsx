...
export interface Zone {
  ...
  signer?: "a" | "b";    // field 用：甲方預填(a) 或 乙方填寫(b，預設)
}
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3 shrink-0 z-20">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium bg-emerald-50 hover:bg-emerald-100 text-emerald-700 transition-colors">
              ...
              <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                ...
                {selected.type === "field" && (
                  <>
                    {/* 填寫方 */}
                    <div>
                      <label className="text-[10px] text-slate-400 font-medium block mb-1">填寫方</label>
                      <div className="flex rounded-lg border border-slate-200 overflow-hidden text-xs font-medium">
                        <button
                          className={`flex-1 py-1.5 transition-colors border-r border-slate-200 ${
                            (selected.signer ?? "b") === "a"
                              ? "bg-orange-100 text-orange-700"
                              : "text-slate-500 hover:bg-slate-50"
                          }`}
                          onClick={() => setZones(prev => prev.map(z => z.id === selectedId ? { ...z, signer: "a" as const } : z))}
                        >
                          甲方預填
                        </button>
                        <button
                          ...
                          onClick={() => setZones(prev => prev.map(z => z.id === selectedId ? { ...z, signer: "b" as const } : z))}
                        >
                          乙方填寫
                        </button>
                      </div>
                    ...
                    <div>
                      <label className="text-[10px] text-slate-400 font-medium block mb-1">
                        {(selected.signer ?? "b") === "a" ? "甲方預填文字" : "預設文字（選填）"}
                      </label>
                      <input
                        className="w-full border border-slate-200 rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-400"
                        placeholder={(selected.signer ?? "b") === "a" ? "請填入甲方文字" : "留空則由乙方填寫"}
