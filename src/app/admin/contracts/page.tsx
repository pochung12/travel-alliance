...
export default function ContractsPage() {
  ...
  const [sendCopied,   setSendCopied]   = useState<"a" | "b" | null>(null);
  ...
  const load = async () => {
    ...
    const [{ data: cData, error: cErr }, { data: tData }, { data: cuData }] = await Promise.all([
      supabase.from("contracts")
        .select("id,title,pdf_name,status,sign_token,sign_token_a,signed_at,signed_at_a,signer_name,signer_name_a,notes,created_at,is_template,tour_id,customer_id,tour:tours(name),customer:customers(name)")
  ...
  const buildEmailHref = (url: string, title: string, partyLabel: string) => {
    const subject = encodeURIComponent(`【暖心旅行社】合約簽署通知 ─《${title}》（${partyLabel}）`);
  ...
  const buildLineHref = (url: string, title: string) => {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
                ...
                <div key={c.id}
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex flex-col gap-0.5 items-end shrink-0">
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium flex items-center gap-0.5 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300">
                        {c.signed_at_a ? <CheckCircle2 className="w-2.5 h-2.5" /> : <Clock className="w-2.5 h-2.5" />}
                        甲方{c.signed_at_a ? "已簽" : "待簽"}
                      </span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium flex items-center gap-0.5 ${STATUS_COLOR[c.status as ContractStatus]}`}>
                        <SIcon className="w-2.5 h-2.5" /> 乙方{STATUS_LABEL[c.status as ContractStatus]}
                      </span>
                    </div>
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400 space-y-0.5">
                    ...
                    {c.signed_at_a && (
                      <p className="text-indigo-600 dark:text-indigo-400">甲 ✓ {c.signer_name_a} · {fmtDate(c.signed_at_a)}</p>
                    )}
                    {c.status === "signed" && c.signed_at && (
                      <p className="text-emerald-600 dark:text-emerald-400">乙 ✓ {c.signer_name} · {fmtDate(c.signed_at)}</p>
                    )}
                  </div>
                  <div className="flex gap-1.5 pt-1 flex-wrap">
                    ...
                    <button onClick={() => { setSendCopied(null); setSendTarget(c); }}
                      ...
                      <tr key={c.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-1">
                            <span className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 w-fit">
                              {c.signed_at_a ? <CheckCircle2 className="w-2.5 h-2.5" /> : <Clock className="w-2.5 h-2.5" />}
                              甲方{c.signed_at_a ? "已簽" : "待簽"}
                            </span>
                            <span className={`inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded-full font-medium w-fit ${STATUS_COLOR[c.status as ContractStatus]}`}>
                              <SIcon className="w-2.5 h-2.5" /> 乙方{STATUS_LABEL[c.status as ContractStatus]}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">
                          {c.signed_at_a && (
                            <p className="text-indigo-600 dark:text-indigo-400">甲 {c.signer_name_a || "（未填）"}<br/><span className="text-slate-400">{fmtDate(c.signed_at_a)}</span></p>
                          )}
                          {c.status === "signed" && c.signed_at
                            ? <p className="text-emerald-600 dark:text-emerald-400 mt-0.5">乙 {c.signer_name || "（未填）"}<br/><span className="text-slate-400">{fmtDate(c.signed_at)}</span></p>
                            : !c.signed_at_a ? <span className="text-slate-300 dark:text-slate-600">尚未簽署</span> : null}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            {/* 發送合約 */}
                            <button onClick={() => { setSendCopied(null); setSendTarget(c); }} title="發送合約"
      ...
      {/* ══ 發送合約 Modal ═══════════════════════════════════════════════════ */}
      {sendTarget && (() => {
        const urlA = sendTarget.sign_token_a ? getSignUrl(sendTarget.sign_token_a) : "";
        const urlB = getSignUrl(sendTarget.sign_token);
        return (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md my-4 overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                {urlA && (
                  <div className="rounded-xl border border-indigo-200 dark:border-indigo-700 bg-indigo-50/50 dark:bg-indigo-900/20 p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-bold text-indigo-700 dark:text-indigo-300 uppercase tracking-wide">甲方（我方）簽署</p>
                      {sendTarget.signed_at_a
                        ? <span className="text-[11px] text-emerald-600 dark:text-emerald-400 flex items-center gap-0.5"><CheckCircle2 className="w-3 h-3" /> 已簽署</span>
                        : <span className="text-[11px] text-indigo-500 flex items-center gap-0.5"><Clock className="w-3 h-3" /> 待簽署</span>}
                    </div>
                    <div className="flex gap-2">
                      <input readOnly value={urlA}
                        className="flex-1 border border-indigo-200 dark:border-indigo-600 rounded-lg px-3 py-2 text-xs text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700 focus:outline-none truncate" />
                      <button onClick={() => { navigator.clipboard.writeText(urlA); setSendCopied("a"); setTimeout(() => setSendCopied(null), 2000); }}
                        className={`px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-1 shrink-0 transition-colors ${
                          sendCopied === "a" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" : "bg-indigo-100 text-indigo-700 hover:bg-indigo-200 dark:bg-indigo-900/40 dark:text-indigo-300"
                        }`}>
                        <Copy className="w-3.5 h-3.5" />{sendCopied === "a" ? "已複製" : "複製"}
                      </button>
                    </div>
                    <div className="flex gap-2">
                      <a href={buildEmailHref(urlA, sendTarget.title, "甲方")} target="_blank" rel="noopener"
                        className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-medium text-xs transition-colors">
                        <Mail className="w-3.5 h-3.5" /> Email 發送
                      </a>
                      <a href={buildLineHref(urlA, sendTarget.title)} target="_blank" rel="noopener"
                ...
                {/* 乙方（客戶）簽署 */}
                <div className="rounded-xl border border-blue-200 dark:border-blue-700 bg-blue-50/50 dark:bg-blue-900/20 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wide">乙方（客戶）簽署</p>
                    {sendTarget.status === "signed"
                      ? <span className="text-[11px] text-emerald-600 dark:text-emerald-400 flex items-center gap-0.5"><CheckCircle2 className="w-3 h-3" /> 已簽署</span>
                      : <span className="text-[11px] text-blue-500 flex items-center gap-0.5"><Clock className="w-3 h-3" /> 待簽署</span>}
                  </div>
                  <div className="flex gap-2">
                    <input readOnly value={urlB}
                      className="flex-1 border border-blue-200 dark:border-blue-600 rounded-lg px-3 py-2 text-xs text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700 focus:outline-none truncate" />
                    <button onClick={() => { navigator.clipboard.writeText(urlB); setSendCopied("b"); setTimeout(() => setSendCopied(null), 2000); }}
                      className={`px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-1 shrink-0 transition-colors ${
                        sendCopied === "b" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" : "bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/40 dark:text-blue-300"
                      }`}>
                      <Copy className="w-3.5 h-3.5" />{sendCopied === "b" ? "已複製" : "複製"}
                    </button>
                  </div>
                  <div className="flex gap-2">
                    <a href={buildEmailHref(urlB, sendTarget.title, "乙方")} target="_blank" rel="noopener"
                    ...
                    <a href={buildLineHref(urlB, sendTarget.title)} target="_blank" rel="noopener"
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-[#06C755] hover:bg-green-600 text-white font-medium text-xs transition-colors">
                      <MessageCircle className="w-3.5 h-3.5" /> LINE 發送
                    </a>
                  </div>
