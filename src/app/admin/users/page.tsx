...
export default function UsersPage() {
  ...
  const load = async () => {
    ...
    const [{ data: prof }, { data: all }] = await Promise.all([
      supabase.from("profiles").select("id,name,role,email,created_at").eq("id", session.user.id).single(),
      supabase.from("profiles").select("id,name,role,email,created_at").order("created_at", { ascending: false }),
    ]);
    ...
    <div className="p-4 md:p-6 space-y-4 md:space-y-5">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
          <UserCog className="w-5 h-5 md:w-6 md:h-6 text-violet-600" /> 用戶管理
        </h1>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="bg-white dark:bg-slate-800 rounded-xl p-3 md:p-4 shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-2 md:gap-3">
            <div className={`p-1.5 md:p-2 rounded-lg ${bg} shrink-0`}>
              <Icon className={`w-4 h-4 md:w-5 md:h-5 ${color}`} />
            </div>
            <div className="min-w-0">
              <div className="text-lg md:text-xl font-bold text-slate-800 dark:text-slate-100">{count}</div>
              <div className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 truncate">{label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="md:hidden space-y-2">
        {profiles.map(p => {
          ...
          return (
            <div key={p.id} className={`bg-white dark:bg-slate-800 rounded-xl border shadow-sm p-4 space-y-3 ${
              isSelf ? "border-blue-200 dark:border-blue-700" : "border-slate-100 dark:border-slate-700"
            }`}>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <div className="font-semibold text-slate-800 dark:text-slate-100 truncate">
                    {p.name || "—"}
                    {isSelf && <span className="ml-1.5 text-xs text-blue-500">（我）</span>}
                  </div>
                  <div className="text-xs text-slate-400 dark:text-slate-500 truncate">{p.email}</div>
                </div>
                <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-medium shrink-0 ${roleInfo.color}`}>
              ...
              {!isSelf && (
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-400">變更角色：</span>
                  {isSaving && <Loader2 className="w-3.5 h-3.5 animate-spin text-slate-400" />}
                  <select
                    value={p.role}
                    disabled={isSaving}
                    onChange={e => handleRoleChange(p.id, e.target.value as UserRole)}
                    className="flex-1 text-xs border border-slate-200 dark:border-slate-600 rounded-lg px-2.5 py-2 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-400 disabled:opacity-50"
                  >
              ...
              <div className="text-[10px] text-slate-400 dark:text-slate-500">
                加入：{new Date(p.created_at).toLocaleDateString("zh-TW")}
              </div>
            </div>
          );
        })}
